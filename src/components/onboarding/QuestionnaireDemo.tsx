...
export default function QuestionnaireDemo({ coachTrack: _coachTrack, onContinue }: Props) {
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-base text-muted-foreground leading-snug">{t('questionnaire.coach.demoSetupBody')}</p>
      ...
      <div className="mb-4 min-h-[1.75rem] text-center text-base text-muted-foreground transition-opacity duration-300">
        {caption}
      </div>

      {/* Stat line — always visible */}
      <div className="mb-6 rounded-xl border-2 border-primary/20 bg-primary/5 p-3.5 text-center">
        <p className="text-sm font-semibold text-primary leading-snug">{t('questionnaire.coach.demoOutputStat')}</p>
      </div>
...
function ParticipantRow({ participant }: { participant: Participant }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-lg border px-3 py-2.5 transition-all duration-500 ${bg}`}>
      <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-bold transition-all ${avatarBg}`}>
        {participant.initials}
      </div>
      <div className="flex-1 text-base font-medium text-foreground">{participant.name}</div>
