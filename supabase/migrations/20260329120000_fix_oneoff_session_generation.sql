-- Fix: generate_sessions_for_training referenced non-existent one_off_date column,
-- causing "session generation failed" for all one-off trainings.
-- The JS code already sets start_date = one_off_date before insert, so just use start_date.
