-- Fix: EXTRACT(ISODOW) returns 1=Monday..7=Sunday, but the frontend
-- stores days_of_week as 0=Monday..6=Sunday (DAYS_FULL array indices).
-- This caused sessions to be generated one day early (Tue+Wed → Mon+Tue).
-- Fix: subtract 1 from ISODOW so it matches the frontend encoding.
...
BEGIN
  ...
  IF v_training.is_recurring THEN
    ...
    WHILE v_target_date <= v_end_date LOOP
      -- ISODOW: Mon=1..Sun=7 → convert to frontend encoding: Mon=0..Sun=6
    ...
    END LOOP;
  ELSE
    -- One-off: start_date holds the one-off date (set by client)
