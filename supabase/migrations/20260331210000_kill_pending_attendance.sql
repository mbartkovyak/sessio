...
-- 3. Fix deduct_abonament_session: remove silent status promotion,
--    and verify the session date falls within the pass validity period.
...
DECLARE
  ...
  v_session_date DATE;
BEGIN
  SELECT pa.sessions_remaining, pa.activated_at, pa.expires_at, pa.school_id, s.owner_id, pa.player_id
  INTO v_remaining, v_activated_at, v_expires_at, v_school_id, v_school_owner_id, v_player_id
  ...
  -- Get session date to validate against pass period
  SELECT ts.session_date INTO v_session_date
  FROM training_sessions ts WHERE ts.id = p_session_id;

  IF v_session_date IS NULL THEN RAISE EXCEPTION 'Session not found'; END IF;

  -- Pass must have been active on the session date
  IF v_activated_at IS NOT NULL AND v_activated_at::date > v_session_date THEN
    RAISE EXCEPTION 'Pass was not active on this session date';
  END IF;

  IF v_expires_at IS NOT NULL AND v_expires_at::date < v_session_date THEN
    RAISE EXCEPTION 'Pass had expired before this session';
  END IF;
...
-- 4. Fix auto_deduct_session: only confirmed, and pass must cover the session date.
...
BEGIN
  ...
  LOOP
    -- Only match passes that were active ON the session date:
    -- activated before/on the session, not expired before the session
